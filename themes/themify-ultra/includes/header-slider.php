...
if (themify_get('header_wrap') === 'slider') {
	...
	$slider_play = themify_get('background_auto', 'yes');
	$slider_autoplay = themify_get('background_autotimeout', '');
	$slider_speed = themify_get('background_speed', 2000);
	$slider_wrap = themify_get('background_wrap', 'yes');

	$slider_args = array(
	    'data-lazy' => 1,
	    'data-effect' => 'fade',
	    'data-auto' => 'yes' === $slider_play ? 1 : 0,
	    'data-speed' => $slider_speed < 2000 ? $slider_speed / 2000 : $slider_speed,
	    'data-wrapvar' => 'yes' === $slider_wrap ? 1 : 0,
	);
	...
	<div id="gallery-controller" class="tf_w tf_h tf_abs tf_overflow">
		<div <?php echo themify_get_element_attributes($slider_args) ?> class="swiper-container tf_carousel tf_overflow tf_h tf_w clearfix" data-height="100%">
