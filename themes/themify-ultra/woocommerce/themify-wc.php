...
class Themify_WC{
	...
	public static function loop_image($image, $product, $size, $attr, $placeholder, $orig_image){
		if ( is_cart() || is_checkout() || wc_get_loop_prop( 'is_shortcode' ) ) {
		    return $image;
		}
		global $themify;
		if($themify->hide_image==='yes'){
		...
		<figure class="post-image product-image">
			...
			<?php if($hoverMode===true):?>
				...
				if(!empty($gallery)){
					...
					echo themify_get_image(array('alt'=>$second_alt,'w'=>$themify->width,'h'=>$themify->height,'image_size'=>$themify->image_size,'src'=>$gallery[0],'class'=>'themify_product_second_image tf_abs tf_opacity'));
				}
				?>
			<?php endif;?>
			<?php if(  $themify->unlink_image!=='yes' ):?>
