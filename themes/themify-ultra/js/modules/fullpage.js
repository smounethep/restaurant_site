    ...
    const updateFullPage = function(options){
            ...
            for(let i = rows.length - 1; i > - 1; -- i){
                ...
                for(let j = childs.length - 1; j > - 1; -- j){
                    if(childs[j].classList.contains(cl)){
                        let child_cl = childs[j].classList;
                        if(childs[j].offsetWidth === 0 && childs[j].offsetHeight === 0){
                            childs[j].parentNode.removeChild(childs[j]);
                        }else if(child_cl.contains('hide-desktop') || child_cl.contains('hide-tablet') || child_cl.contains('hide-mobile')){
                            for(let key in bpRange){
                                if(bpRange[key] === true && child_cl.contains('hide-' + key)){
                                    childs[j].parentNode.removeChild(childs[j]);
                                    break;
                                }
                            }
        ...
        scrollTo = function(el,anchor){
            ...
            if(anchor && '#' !== anchor){
                ...
                if(sectionEl !== null && ! sectionEl.hasAttribute('data-hide-anchor')){
                    sectionEl = $(sectionEl);
        ...
        createFullScrolling = function(options){
            ...
            if(rows && rows[0]){
                ...
                for(let i = 0,len = rows.length; i < len; ++ i){
                    let section_anchor = getAnchor(rows[i]);
                    if(section_anchor){
        ...
        _resizer = function(e){
            ...
            if(initFullPage===true && isMobile){
                ...
                Themify.body[0].classList.add('full-section-scrolling');
                Themify.lazyDisable=true;
