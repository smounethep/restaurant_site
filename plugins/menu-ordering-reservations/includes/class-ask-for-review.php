...
if (!class_exists('Glf_Ask_For_Review')) {
    ...
    class Glf_Ask_For_Review {
        ...
        public function glf_action_ask_for_review_user_response() {
            ...
            $this->glf_database_option_action('update', self::DB_OPTION_ASK_FOR_REVIEW, $this->db_option_ask_for_review);
            echo json_encode(array('message' => 'done'));
            exit;
        }

        private function glf_get_database_option() {
            //$this->glf_database_option_action( 'delete', self::DB_OPTION_ASK_FOR_REVIEW );
            $db_option = $this->glf_database_option_action('get', self::DB_OPTION_ASK_FOR_REVIEW, '');
            if (empty($db_option)) {
                $this->glf_database_option_action('add', self::DB_OPTION_ASK_FOR_REVIEW, $this->default_option_ask_for_review);
                $db_option = $this->glf_database_option_action('get', self::DB_OPTION_ASK_FOR_REVIEW, '');
        ...
        private function glf_database_option_action($action, $option_name, $value = '') {
            $result = '';
            if ($action === 'get') {
